...
-- [回答]
SELECT store_code FROM stock_table WHERE quantity >= (SELECT AVG(quantity) 
...
VALUES ('M002', 'マフラー', 4500, '1000-01-01');

-- 7. 在庫テーブルの商品コード='S987'、かつ、店舗コード='EA01'に対して、「在庫数=10、更新日付=本日日付」で更新しなさい。※実行後のSELECT結果も貼付すること。
-- [回答]
UPDATE stock_table 
SET
quantity = 10, update_day = current_timestamp
WHERE goods_code = 'S987'AND store_code = 'EA03';
